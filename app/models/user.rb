...
class User < ActiveRecord::Base
  ...
  after_update :save_services
  after_update :save_testimonials
  ...
  def experience_attributes=(experience_attributes)
      experience_attributes.each do |attributes|
        if attributes[:id].blank?
          experiences.build(attributes)
        else
          experience = experiences.detect { |t| t.id == attributes[:id].to_i }
  ...
  def service_attributes=(service_attributes)
      service_attributes.each do |attributes|
        if attributes[:id].blank?
          services.build(attributes)
        else
          service = services.detect { |t| t.id == attributes[:id].to_i }
  ...
  def testimonial_attributes=(testimonial_attributes)
      testimonial_attributes.each do |attributes|
        if attributes[:id].blank?
          testimonials.build(attributes)
        else
          testimonial = testimonials.detect { |t| t.id == attributes[:id].to_i }
          testimonial.attributes = attributes
        end
      end
  end
